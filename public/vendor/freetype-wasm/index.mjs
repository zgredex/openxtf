/**
 * freetype-wasm — a thin general-purpose JavaScript wrapper for FreeType.
 *
 * Two usage layers:
 *  1) Convenience layer: the FreeType / Face classes cover common workflows
 *     (open font → set size → render glyph → read bitmap and metrics).
 *     The caller chooses MONO or grayscale antialiasing through load flags.
 *  2) Native layer: `ft.module` exposes the complete Emscripten module through
 *     `ccall/cwrap/getValue/setValue/HEAPU8/_malloc/_free/addFunction`, providing
 *     direct access to any public FreeType C function. Use it with `ft.offsets`
 *     for advanced operations such as outline extraction, size management, and
 *     module properties that are not wrapped by the convenience layer.
 *
 * Required sibling build artifacts: freetype.mjs, freetype.wasm, and offsets.mjs.
 */
import OFFSETS from "./offsets.mjs";

// Common stable constants from FreeType's public headers.
export const FT = {
  // FT_LOAD_*
  LOAD_DEFAULT: 0x0,
  LOAD_NO_SCALE: 0x1,
...
// wasm32: long, FT_Pos, FT_Fixed, int, and pointers are four bytes.
const I32 = "i32";

let _factory; // Cached dynamic import.
...
export class FreeType {
  constructor(mod) {
    this.module = mod; // Native escape hatch: the complete Emscripten module.
    ...
    if (err) {
      mod._free(libPP);
      throw new Error(`FT_Init_FreeType failed: ${err}`);
    }
    this.library = mod.getValue(libPP, I32);
    mod._free(libPP);
  }

  /** FreeType version as [major, minor, patch]. */
  version() {
    const m = this.module;
  ...
  /** Create a Face from font bytes. WOFF2 support includes Brotli. */
  newFace(bytes, faceIndex = 0) {
    ...
    if (err) {
      m._free(dataPtr);
      throw new Error(`FT_New_Memory_Face failed: ${err}`);
    }
    return new Face(this, facePtr, dataPtr); // FreeType does not copy the bytes; keep dataPtr alive with the Face.
  }
...
export class Face {
  ...
  selectCharmap(encoding) {
    ...
    if (e) throw new Error(`FT_Select_Charmap failed: ${e}`);
    return this;
  }
   ...
   *   char: Unicode code point; index: direct glyph index; flags defaults to LOAD_DEFAULT.
   *   render defaults to true and renderMode defaults to NORMAL grayscale AA.
   *   Pass FT.RENDER_MODE_MONO for 1-bit output.
   * @returns {{width,rows,pitch,pixelMode,numGrays,bitmapLeft,bitmapTop,advance,
   *            metrics, buffer:Uint8Array}} The buffer is copied from the Wasm heap.
   */
  loadGlyph(o = {}) {
    const m = this.module;
    const O = this._O;
    ...
    if (e) throw new Error(`FT_Load_${o.index != null ? "Glyph" : "Char"} failed: ${e}`);

    const slot = this._faceField("glyph"); // FT_GlyphSlotRec pointer.
    if (o.render !== false && !(flags & FT.LOAD_RENDER)) {
      e = this.ft._fn.RenderGlyph(slot, o.renderMode ?? FT.RENDER_MODE_NORMAL);
      if (e) throw new Error(`FT_Render_Glyph failed: ${e}`);
    }
    ...
    const pitch = m.getValue(bmp + B.pitch, I32); // Signed; negative means bottom-up rows.
    ...
    return {
      ...
      advance: {
        x: m.getValue(adv + O.FT_Vector.x, I32), // 26.6 fixed point.
      ...
      buffer, // MONO: 1bpp, |pitch| bytes per row, MSB first; GRAY: 8bpp.
    };
  }

  /** Kerning between two glyph indices in 26.6 units; requires a kern table. */
